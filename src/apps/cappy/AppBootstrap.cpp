...
AppBootstrap::AppBootstrap(QApplication& app)
    : app_(app), directories_(AppDirectories::ensureCreated()),
...
int AppBootstrap::run() {
    ...
    singleInstanceLock_ =
        std::make_unique<QLockFile>(QDir(directories_.appDataDir).filePath("cappy.instance.lock"));
    singleInstanceLock_->setStaleLockTime(0);
    if (!singleInstanceLock_->tryLock()) {
        const auto shellSettings = settings_->loadShellSettings();
        const auto language =
            cappy::localization::resolvedAppLanguageFromSettings(shellSettings.interfaceLanguage);
        const auto& text = cappy::localization::strings(language);
        qWarning() << "Another Cappy instance is already running.";
        QMessageBox::warning(nullptr, text.appName, text.singleInstanceAlreadyRunningMessage);
        return 1;
    }
    ...
    const bool activeWindowCaptureSmokeTest =
        app_.arguments().contains("--smoke-active-window-capture");
    ...
    controller_ = std::make_unique<AppController>(app_, *mainWindow_, *settings_, logFilePath);
    controller_->initialize();
    QObject::connect(&app_, &QCoreApplication::aboutToQuit, controller_.get(),
